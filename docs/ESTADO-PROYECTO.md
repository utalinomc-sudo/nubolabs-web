...
7. ~~**Migrar a Next 16 y firebase-admin 14**~~ ✅ **Hecho el 2026-09-16** (cambio `migrar-next-16`, rama `feature/migrar-next-16`, mezclada a `main` el 2026-09-16 tras validar el preview de Vercel con login real; archivado en `openspec/changes/archive/2026-09-16-migrar-next-16/`): `next` 14.2.35 → **16.3.5** (Turbopack por defecto en `dev` y `build`), `firebase-admin` 12.7 → **14.4.0**, `eslint` 8 → **9.39** con flat config (`eslint.config.mjs`; `npm run lint` = `eslint .`, `next lint` desapareció), `cookies()` y `params` asíncronos en `lib/adminAuth.ts` y las 4 rutas con `[id]`, `data-scroll-behavior="smooth"` en el `<html>` (Next 16 ya no anula el scroll suave global al navegar) y `agentRules: false` en `next.config.js` (para que `next dev` no inserte su bloque en `AGENTS.md`/`CLAUDE.md`). `npm audit --omit=dev` pasó de 10 alertas (1 crítica, 1 alta, 8 moderadas) a **2 moderadas residuales** sin parche (pendiente 8). Lint, tipos, 20 tests (11 + 9 de `lib/privateKey.test.ts`), build, `curl` a los 11 endpoints (idénticos a la línea base) y E2E completo con Playwright MCP en 1280 y 375 px en verde; reportes y capturas en `openspec/changes/archive/2026-09-16-migrar-next-16/reports/`. Nota: el App Router de Next 16 corre con el **React 19.3 canary que empaqueta Next**; `react` 18.3 sigue instalado (tests con Testing Library).  Tras el primer preview (error 500 en todas las páginas dinámicas) se añadió `lib/privateKey.ts` con su test: firebase-admin 14 rechaza la clave privada guardada con comillas, en una sola línea o con espacios sobrantes (la 12 lo toleraba); ahora se normaliza y un fallo de inicialización deja el sitio como sin credenciales, con aviso `[firebase-admin]` en el log, en vez de tumbarlo (reporte `2026-09-16-paso-2b-clave-privada.md`). El segundo preview mostró la causa real: `Failed to load external module firebase-admin…/auth: ERR_REQUIRE_ESM` (`jwks-rsa` hace `require()` de `jose` 6, solo ESM, que exige Node ≥ 20.19/22.12) ; `firebase-admin/auth` ahora se carga de forma perezosa y protegida (el sitio público no lo necesita). Node en Vercel ya estaba en 24.x: el tercer preview, redesplegado **sin caché de build**, funcionó por completo (login real, leads, PDF, CMS), así que el fallo se atribuye a una caché de build heredada de Next 14 (reportes `2026-09-16-paso-2c-auth-esm.md` y `2026-09-16-paso-5-preview.md`).
...
11. **Tras mezclar `migrar-next-16` a `main`: redesplegar producción sin caché de build y comprobar el login real.** El primer deploy de producción reutiliza la caché de build de los despliegues con Next 14, la misma condición sospechosa de los dos previews que fallaron (reporte `2026-09-16-paso-5-preview.md`). Pasos del dueño: en Vercel, "Redeploy" del deployment de producción con "Use existing Build Cache" desmarcado; entrar en `/admin/login` con el usuario real y abrir el detalle de un lead. Si responde `500 Auth no configurado`, revisar el log de funciones (aviso `[firebase-admin] no se pudo cargar firebase-admin/auth`) y repetir el redeploy sin caché. El sitio público no depende de ese módulo y seguiría en pie. Hallazgo de `/adversarial-review` (2026-09-16); tarea 5.3 del cambio, que se completa después de archivar. **Confirmado el 2026-09-16 tras el merge (`0eae29b`):** el primer deploy automático de producción sirve Next 16 y Firestore (portada, `/equipo` con el equipo real, `/admin` redirige al login), pero `POST /api/admin/session` responde `500 Auth no configurado` con un token de prueba: el módulo de auth no cargó, como se previó. **Acción pendiente del dueño: redeploy sin caché de build y login real**; al informarlo, cerrar este pendiente y la fila de la tabla "Tras el merge" del reporte del paso 5.
