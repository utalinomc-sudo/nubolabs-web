...
- Pendientes detectados en el bootstrap (perfil §11): solo queda elegir e instalar el runner de tests (en curso como cambio OpenSpec). Resueltos el 2026-09-15: dominio en Resend, versión de Node (24.x), política de PRs (solo push), idioma de identificadores (inglés) y `metadataBase` corregido a `https://www.nubolabs.cl` en `app/layout.tsx`.
