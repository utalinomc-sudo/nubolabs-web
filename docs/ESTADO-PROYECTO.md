...
10. **Fijar Node.js 24.x en Vercel** (Settings → Build and Deployment → Node.js Version) y confirmarlo en el log de build del siguiente despliegue. `firebase-admin` 14 exige Node ≥ 22 y su módulo de auth necesita `require(esm)` (Node ≥ 20.19 / 22.12); el preview del 2026-09-16 corría con un Node anterior y el login del panel no funcionaría (`500 Auth no configurado`, aviso `[firebase-admin]` en los logs) aunque el sitio público sí. `package.json` ya declara `engines.node: "24.x"`; si Vercel lo ignora, revisar el ajuste del proyecto.
