...
- Email: the `nubolabs.cl` domain is verified in Resend (account `utalinomc@gmail.com`, since 2026-09-15) and Vercel has `NOTIFY_FROM` = `avisos@nubolabs.cl` and `REPORT_FROM` = `informe@nubolabs.cl`. Any sender outside a verified domain (e.g. `onboarding@resend.dev`) only reaches the account owner. Notification recipient defaults to `mauricio.nubolabs@gmail.com` (`NOTIFY_EMAIL`).
...
- Unit tests run with Vitest (`npm test`, co-located `*.test.ts`, `node` environment). Test only the pure parts of an integration (payload building, normalization, the `skipped` result when the credential is missing), mocking `fetch` when needed; never call the real service from a test. Verification is `npm run lint`, `npx tsc --noEmit`, `npm test`, `npm run build` plus a documented manual test with the variable present and absent (to prove degradation). Never send real emails to clients or upload to production Blob during tests without explicit authorization.
