...
- Identify the current branch. Follow the branching policy in `docs/project-profile.md` §7: if it requires feature branches and you are on the main branch, create one before committing; if it allows small changes directly on the main branch, ask the user which they prefer.
...
- Write the commit message in the language defined in `docs/project-profile.md` §6 (Commits / PRs).
...
- Skip this step if `docs/project-profile.md` §7 says pull requests are not used (just report the pushed branch). Otherwise create or update the PR for the current branch:
...
- `docs/project-profile.md` §6 (languages) and §7 (branching, commit convention, PR policy).
