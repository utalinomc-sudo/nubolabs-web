...
- Unit tests run with Vitest (`npm test`): co-located `*.test.ts(x)`, explicit imports from `vitest`. Pure logic (e.g. `components/diagnostico/ahorro.ts`, tested in `ahorro.test.ts`) uses the default `node` environment; component tests start with `// @vitest-environment jsdom` and use Testing Library (`@testing-library/react`, jest-dom matchers preloaded by `vitest.setup.ts`). Verification is `npm run lint`, `npx tsc --noEmit`, `npm test`, `npm run build` plus an E2E pass with Playwright MCP against `npm run dev` (works without Firebase credentials: leads are not persisted, `/admin` opens with a dev session).
