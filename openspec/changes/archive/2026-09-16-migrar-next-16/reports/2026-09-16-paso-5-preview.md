...
| 2026-09-16 14:53 | deploy automático del fix `a1ea8a7` (`overrides` de jose) | no necesario | `POST /api/admin/session` con token falso → **401** `{"error":"No se pudo crear la sesión."}` (módulo de auth cargado; antes 500) · `/` 200 · `/equipo` con los 4 integrantes · `/admin` sin cookie → 307 | pendiente | **PASS técnico.** Queda el login real del dueño y abrir un lead; al confirmarlo se cierra el pendiente 11 de `ESTADO-PROYECTO.md` |
