...
13. **Normalización de `FIREBASE_PRIVATE_KEY` e inicialización protegida (añadido el 2026-09-16 tras el primer preview).** El preview de Vercel devolvía error de servidor en todas las páginas dinámicas mientras el build de producción local funcionaba con y sin credenciales. Prueba con `cert()` de ambas versiones y una clave RSA generada al vuelo: firebase-admin 12 (node-forge) acepta la clave entre comillas dobles, en una sola línea con espacios o con espacios al inicio/fin; firebase-admin 14 (`crypto` nativo, cambio de 13.10) rechaza esas tres formas con "Failed to parse private key". `.env.local.example` pide pegar la clave entre comillas, por lo que el valor guardado en Vercel muy probablemente las incluye. Como `getDb()`/`getAdminAuth()` crean la app fuera del `try/catch` de `lib/site.ts`, el error de `cert()` tumba la página. Decisión: (a) nueva función pura `normalizePrivateKey` en `lib/privateKey.ts` (sin `server-only`, testeable en Vitest) que quita comillas envolventes, convierte `\n` literales y CRLF, y reconstruye el PEM en su forma canónica (cabecera, cuerpo en líneas de 64, pie) aunque venga en una sola línea; (b) `getAdminApp()` captura cualquier error de `cert()`/`initializeApp()`, lo registra con prefijo `[firebase-admin]` y devuelve `null` (memorizado), de modo que el sitio degrada como sin credenciales en vez de caer; (c) nota en `.env.local.example` de que en Vercel el valor va sin comillas. Alternativa descartada: solo corregir el valor en Vercel, porque no evita que un pegado futuro vuelva a tumbar el sitio y deja el fallo sin diagnóstico en los logs. Se verifica con el test unitario (todas las variantes pasan por `crypto.createPrivateKey`), con `next start` local usando una clave falsa entre comillas (antes: error de servidor; después: 200 con degradación) y con el preview de Vercel.
14. **Carga perezosa y protegida de `firebase-admin/auth` (añadido tras el segundo preview, 2026-09-16).** El log de funciones de Vercel mostró la causa real del error 500: `Failed to load external module firebase-admin-<hash>/auth: Error [ERR_REQUIRE_ESM]`. `firebase-admin/auth` (CJS) depende de `jwks-rsa` 4.1, que hace `require("jose")`, y `jose` 6 es solo ESM; ese `require()` de un módulo ESM solo funciona en Node ≥ 20.19 o ≥ 22.12 (lo que `jwks-rsa` declara en `engines`), y en Node 24 local, en Node 20.20 y en el simulacro `--no-experimental-require-module` se reproduce exactamente: el comportamiento corresponde a un runtime sin `require(esm)`. **Resultado real (2026-09-16):** Node.js Version en Vercel ya estaba en 24.x; el tercer preview (`385accd`, redesplegado **sin caché de build**) funcionó por completo, login incluido, así que el fallo anterior se atribuye con más probabilidad a una caché de build heredada de Next 14 / firebase-admin 12 que al ajuste de versión (no verificable desde fuera). Se mantiene 24.x como requisito documentado. Como defensa en el código (principio "nunca rompas el sitio por una integración opcional"): `lib/firebaseAdmin.ts` deja de importar `firebase-admin/auth` en la cabecera y lo carga con `import()` dentro de `getAdminAuth()` (ahora asíncrona) protegido con `try/catch` y fallo memorizado con log `[firebase-admin]`; el sitio público solo necesita `firebase-admin/app` y `/firestore` (cadena CJS pura), así que sigue funcionando aunque el módulo de auth no cargue, y el panel responde `500 Auth no configurado` en vez de tumbar todas las páginas. Callers: `lib/adminAuth.ts` y `app/api/admin/session/route.ts` pasan a `await getAdminAuth()`. Alternativas descartadas: `overrides` de `jose` a la 5.x (API distinta de la que `jwks-rsa` 4 declara, riesgo en la verificación de tokens) y bundlear `firebase-admin` con `transpilePackages` (arrastra gRPC/protobuf al bundle). Verificación: `next start` con `--no-experimental-require-module` antes (`/` 500) y después (`/` 200, `POST /api/admin/session` 500 controlado con log), más los cuatro comandos y la matriz de `curl`.
...
- [`firebase-admin` 14 rechaza el formato con el que está guardada `FIREBASE_PRIVATE_KEY` en Vercel (comillas, una sola línea, espacios) y `cert()` tumba todas las páginas dinámicas] → decisión 13: `normalizePrivateKey` + inicialización protegida con log `[firebase-admin]`; comprobado en el preview.
