...
- **Código de aplicación:** los 6 archivos listados en What Changes (`lib/adminAuth.ts`, 4 rutas dinámicas, `app/layout.tsx`) más `next.config.js`; se crea `eslint.config.mjs` y se elimina `.eslintrc.json`. `next-env.d.ts` está ignorado por git y lo regenera Next.
