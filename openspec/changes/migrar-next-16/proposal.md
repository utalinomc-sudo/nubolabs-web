...
- Sin variables de entorno nuevas. `.env.local.example` solo gana un comentario (en Vercel la clave privada va sin comillas, decisión 13); sus variables no cambian.
...
- **Cuando** se mezcla la rama a `main` y Vercel despliega producción, **entonces** el dueño redespliega producción **sin caché de build** (el primer deploy reutilizaría la caché de los builds con Next 14, la condición sospechosa de los dos previews fallidos) e inicia sesión en `/admin/login` con su usuario real y abre un lead; si respondiera `500 Auth no configurado`, repite el redeploy sin caché y revisa el log `[firebase-admin]` (tarea 5.3; `docs/ESTADO-PROYECTO.md`, pendiente 11).
